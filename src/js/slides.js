...
function render(){
  ...
  const slides=[];
    if(state.bullTimes.length>0){
      ...
      for(let row=0; row<5; row++){
        ...
        if(right){
          const idx=row+5;
          html+=`<td>${idx+1}. <span class="team-${state.players[right.name]}"> ${right.name} </span> - <span class="time">${right.time}s</span></td>`;
        }else{html+='<td></td>';}
        html+='</tr>';
      }
      html+='</table></div>';
  ...
  if(state.bingoWinners){
    ...
    rows.forEach((r,i)=>{
      ...
      html+=`<tr class="${cls}"><td>${r.pos} <span class="team-${state.players[r.name]}"> ${r.name} </span> (<span class="points">${pts}</span> pts) ${r.trophy||''}</td></tr>`;
    });
    html+='</table></div>';
  ...
  scoreEntries.forEach(([team,score],i)=>{
    ...
    html+=`<div class="score-row"><div class="score-bar team-${team}" style="width:${pct}%"><span class="score-name"> ${state.teamNames[team]} </span><span class="score-value points"> ${score}${i==0?' 🏆':''} </span></div></div>`;
  });
  html+='</div></div>';
